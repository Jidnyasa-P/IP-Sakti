...
export const TraditionalKnowledgeView: React.FC<TraditionalKnowledgeViewProps> = ({ onOpenCitation }) => {
  ...
  const [formData, setFormData] = useState<TKABSQuery>({
    ...
    traditional_use: 'Rasayana (rejuvenative), balya (strength-promoting), and stress management in classical Ayurveda (Charaka Samhita)',
    source_community_info: 'Procured from registered farmer cooperatives and cultivated farm estates with BMC records',
  ...
  const plantPresets = [
    ...
    },
    {
      ...
      community: 'Forest department licensed tapping through tribal cooperatives',
      intended: 'Commercial utilization' as const,
    },
  ];
  ...
  return (
    <div className="w-full max-w-7xl xl:max-w-[1440px] 2xl:max-w-[1560px] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      {/* Title & Introduction */}
      <div className="space-y-2">
        ...
        <p className="text-sm text-slate-600 max-w-3xl">
          {t('tk.subtitle', 'Evaluate biological resource compliance under the Biological Diversity Act 2002 and 2023 Amendments, National Biodiversity Authority (NBA) approval requirements, and Traditional Knowledge Digital Library (TKDL) prior art bars.')}
        </p>
      </div>
      ...
      <div className="p-6 sm:p-8 rounded-2xl bg-white border border-slate-200 shadow-xs space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              onChange={(e) => setFormData({ ...formData, plant_material: e.target.value })}
              className="w-full px-3.5 py-2 text-sm rounded-lg border border-slate-300 focus:ring-1 focus:ring-emerald-700 focus:outline-hidden"
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              onChange={(e) => setFormData({ ...formData, geographic_origin: e.target.value })}
              className="w-full px-3.5 py-2 text-sm rounded-lg border border-slate-300 focus:ring-1 focus:ring-emerald-700 focus:outline-hidden"
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              onChange={(e) => setFormData({ ...formData, intended_use: e.target.value as any })}
            ...
            >
              <option value="Commercial utilization">Commercial manufacturing in India (Section 7 SBB)</option>
              <option value="IP filing">Applying for Patent / IPR (Section 6 NBA Form III)</option>
              <option value="Foreign entity utilization">Foreign Entity / NRI / Foreign Collaboration (Section 3 NBA Form I)</option>
              <option value="Academic research">Pure Academic Research (Section 5 exemption)</option>
            </select>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            onChange={(e) => setFormData({ ...formData, traditional_use: e.target.value })}
            className="w-full px-3.5 py-2 text-sm rounded-lg border border-slate-300 focus:ring-1 focus:ring-emerald-700 focus:outline-hidden"
          />
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            onChange={(e) => setFormData({ ...formData, source_community_info: e.target.value })}
            className="w-full px-3.5 py-2 text-sm rounded-lg border border-slate-300 focus:ring-1 focus:ring-emerald-700 focus:outline-hidden"
          />
        </div>
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
          ...
          <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-xs space-y-3">
            ...
            <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-700">
              <strong>Prior Art Status:</strong> {result.prior_art_tk_considerations}
            </div>
          </div>
          ...
          <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-xs space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2 text-xs">
                <span className="font-bold text-slate-900 block">Required Statutory Approvals</span>
                <div className="space-y-1 text-slate-700">
                  <div className="flex items-center gap-2">
                    <span className="text-slate-500">NBA Approval Required:</span>
                    <strong className={result.abs_considerations.nba_approval_needed ? 'text-rose-700' : 'text-emerald-700'}>
                      {result.abs_considerations.nba_approval_needed ? 'YES (Form I or Form III)' : 'No'}
                    </strong>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-slate-500">SBB Intimation Required:</span>
                    <strong>{result.abs_considerations.sbb_notification_needed ? 'YES (State Board)' : 'No'}</strong>
                  </div>
                </div>

                <div className="pt-2 border-t border-slate-200">
                  <span className="text-slate-500 block mb-1">Applicable Sections:</span>
                  <ul className="space-y-1 text-[11px] text-slate-700">
                    {result.abs_considerations.statutory_sections.map((s, i) => (
                      <li key={i}>• {s}</li>
                    ))}
                  </ul>
                </div>
              ...
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2 text-xs">
                <span className="font-bold text-slate-900 block">Benefit Sharing & Exemptions</span>
                <p className="text-slate-700">
                  <strong>Levy Matrix:</strong> {result.abs_considerations.benefit_sharing_rate}
                </p>
                <div className="p-2.5 rounded-lg bg-emerald-50 text-emerald-900 border border-emerald-200/60 mt-2">
                  ...
                  <p className="text-[11px] mt-0.5">{result.abs_considerations.exemptions_applicable}</p>
                </div>
              </div>
            </div>
          </div>
