...
function getAI(): GoogleGenAI | null {
  if (!aiInstance && process.env.LLM_API_KEY) {
...
export async function generateRAGAnswer(
  ...
  const statutoryContext = contextChunks
    .map(
      (c, i) =>
...
Return ONLY valid JSON.`;
    ...
    try {
      ...
      if (response.text) {
        ...
        return {
          ...
          relevant_considerations: Array.isArray(parsed.relevant_considerations)
            ? parsed.relevant_considerations
            : [],
      ...
      console.warn(
        "Gemini API call failed or timed out, using grounded statutory fallback:",
        err,
      );
    }
  }
  ...
  const isPatents =
    query.toLowerCase().includes("patent") ||
    query.toLowerCase().includes("invent") ||
  ...
  const isNBA =
    ...
    query.toLowerCase().includes("abs");
  const isGMP =
    query.toLowerCase().includes("schedule t") ||
    query.toLowerCase().includes("gmp") ||
    query.toLowerCase().includes("manufacturing") ||
