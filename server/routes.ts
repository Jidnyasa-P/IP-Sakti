...
} from "./data/authoritative_documents.js";
import { analyzeFormulation } from "./rag/grounded.js";
...
let savedProducts: any[] = [
  {
    ...
    product_information: {
      ...
      ingredients:
        "Withania somnifera (Ashwagandha) root extract 300mg, Curcuma longa (Haldi) rhizome extract 150mg, Ocimum sanctum (Tulsi) leaf extract 100mg",
      classical_reference:
        "Formulation inspired by Rasayana principles in Charaka Samhita Chikitsa Sthana",
      manufacturing_info:
        "Aqueous hydro-alcoholic extraction following Schedule T GMP standards",
      intended_use:
        "Immune support, vitality, stress mitigation and general adaptogenic tonic",
      claims:
        "Enhances cognitive stamina, balances Vata-Kapha, reduces oxidative stress",
      target_market: "Both",
    },
    likely_category: "Proprietary Ayurvedic Medicine",
    category_reasoning:
      "Utilizes classical Ayurvedic herbs in non-classical proprietary ratios and modern dosage form, falling squarely under Section 3(h) of Drugs and Cosmetics Act for Patent and Proprietary Ayurvedic Medicine.",
    confidence: {
      level: "High",
      score: 0.93,
      reasons: [
        "Ingredients documented in Ayurvedic Pharmacopoeia of India (API)",
        "Clear alignment with Schedule T GMP",
      ],
    },
    regulatory_considerations: [
      {
        ...
        description:
          "Requires State Licensing Authority (SLA) Form 25D license under Rule 154 of Drugs and Cosmetics Rules.",
        governing_statute: "Drugs and Cosmetics Act, 1940 & Rules 1945",
        actionable_requirement:
          "Submit product master dossier, stability protocol, and safety evidence to State AYUSH SLA.",
      },
      {
        title: "Schedule T GMP Compliance",
        description:
          "Premises, raw material quarantine, and analytical quality control must comply with Schedule T.",
        governing_statute: "Schedule T (Drugs and Cosmetics Rules)",
        actionable_requirement:
          "Perform heavy metal, pesticide, and microbial limit testing for every commercial batch.",
      },
    ],
    ipr_considerations: {
      patent_assessment:
        "Composition as a whole faces severe Section 3(p) and 3(e) patentability rejections. Proprietary extraction process may be patentable if novel and inventive.",
      section_3p_tk_bar:
        "High Risk. Ashwagandha, Turmeric, and Tulsi are extensively documented in CSIR-TKDL.",
      section_3e_admixture_bar:
        "High Risk. Must submit comparative synergy assay proving super-additive cellular antioxidant efficacy.",
      trademark_recommendation:
        'Register coined brand "AyurShakti" in Class 5 (pharmaceuticals/herbal) and Class 3 (cosmetics).',
      industrial_design:
        "Protect unique bottle ergonomics and embossed Ayurvedic emblem under Designs Act, 2000.",
      trade_secret_potential:
        "Maintain exact multi-stage extraction temperatures, solvent ratios, and bioactive marker standardization as confidential know-how.",
    },
    traditional_knowledge_abs_flags: {
      tk_prior_art_risk: "High",
      tk_details:
        "Documented in classical Sanskrit texts Charaka Samhita and Bhavaprakasha.",
      biological_resource_status:
        "All three botanical components are Indian biological resources subject to NBA oversight.",
      nba_abs_requirements:
        "Form III approval mandatory prior to filing any patent application. Form I required if exporting raw herbs or with foreign investment.",
      form_required:
        "Form III (IPR Approval) & Form I (Export / Commercial Access)",
    },
    recommended_next_steps: [
    ...
    evidence: [
      {
        ...
        source: "Official Gazette of India",
        excerpt:
          "Traditional knowledge exclusion prohibits patenting known medicinal plants or combinations without inventive step...",
      },
    ],
...
apiRouter.post("/chat", async (req: Request, res: Response) => {
  try {
    ...
    if (!userQuery) {
      return res
        .status(400)
        .json({ error: "Query or message string is required." });
    }

    const startTime = Date.now();
    const grounded = await getGroundedAnswer(
      userQuery,
      language,
      conversation_id,
    );
    ...
    telemetry.average_retrieval_latency_ms = Math.round(
      telemetry.average_retrieval_latency_ms * 0.8 + latency * 0.2,
    );
    ...
    res
      .status(500)
      .json({
        error: "Failed to process statutory query",
        details: err.message,
      });
  }
});
...
apiRouter.post("/chat/stream", async (req: Request, res: Response) => {
  try {
    ...
    const retrieval = getGroundedAnswer(userQuery, 3);
    const ragAnswer = await getGroundedAnswer(
      userQuery,
      grounded.chunks,
      language,
    );
    ...
    const words = grounded.content.split(" ");
...
apiRouter.post("/products/analyze", (req: Request, res: Response) => {
  ...
  const likelyCategory = isProprietary
    ...
    regulatory_considerations: [
      {
        ...
        description:
          "Requires state drug licensing approval with proof of safety/efficacy or classical text citation.",
        governing_statute: "Drugs & Cosmetics Act, 1940 & Rules 1945",
        actionable_requirement:
          "Compile stability test data and submit application to State AYUSH Licensing Authority.",
      },
      {
        title: "Schedule T GMP Certification",
        description:
          "Facility must satisfy Schedule T spatial segregation, HVAC, and QC laboratory requirements.",
        governing_statute: "Schedule T (Drugs and Cosmetics Rules)",
        actionable_requirement:
          "Perform heavy metal (Pb, Cd, As, Hg), pesticide residue, and microbial limit testing per batch.",
      },
    ],
    ipr_considerations: {
      patent_assessment:
      ...
      industrial_design:
        "File for Design registration under the Designs Act, 2000 for unique bottle shapes, packaging ergonomics, or dispenser caps.",
      trade_secret_potential:
        "Keep temperature curves, fractionation solvents, and standardization ratios strictly as trade secrets with non-disclosure agreements (NDAs).",
    },
    traditional_knowledge_abs_flags: {
      ...
      biological_resource_status:
        "Indian biological resource subject to Biological Diversity Act, 2002.",
      ...
      form_required: analysis.isExport
        ? "Form I (Export / Access) & Form III (IPR)"
        : "Form III (IPR) & SBB Form I",
    },
    recommended_next_steps: [
    ...
    evidence: [
      {
        ...
        source: "Official Gazette of India",
        excerpt:
          "Inventions which in effect are traditional knowledge are barred from patentability...",
      },
      {
        index: 2,
        chunk_id: "CHUNK-BD-001",
        ...
        source: "Gazette of India",
        excerpt:
          "Prior approval of the National Biodiversity Authority is required before applying for IPR...",
      },
    ],
    created_at: new Date().toISOString(),
  };
...
apiRouter.post("/ipr/analyze", (req: Request, res: Response) => {
  ...
  if (assetType.includes("formulation") || assetType.includes("invention")) {
    potentialProtection = [
      "Patent (Process only with synergy data)",
      "Trade Mark (Brand Name)",
      "Trade Secret (Extraction parameters)",
    ];
    primaryProtection =
      "Trade Mark & Trade Secret Strategy (with conditional Process Patent)";
    whyRelevant =
      "Product formulations based on known herbs face Section 3(p) and 3(e) statutory bars under the Patents Act, 1970. A strong distinctive Trademark provides commercial exclusivity without patent vulnerability.";
    ...
    potentialProtection = [
      ...
      "Copyright (for stylized artistic logo)",
    ];
    primaryProtection = "Trade Mark (Class 5 & Class 35)";
    whyRelevant =
      "Trademarks grant 10-year renewable legal monopoly over brand name, preventing counterfeiting and misleading consumer confusion.";
    importantConsiderations = [
    ...
    documentsToPrepare = [
      ...
      "Clear representation of logo/mark",
      "User affidavit with first date of commercial use",
    ];
    possibleNextSteps = [
      "File trademark application online via IP India gateway",
      "Monitor for examination report within 1-3 months",
    ];
  } else {
    potentialProtection = [
      "Trade Mark",
      "Designs Act, 2000 (Packaging/Bottle)",
      "Trade Secret",
    ];
    primaryProtection = "Integrated IP Portfolio (Trademark + Design)";
    whyRelevant =
      "Provides multi-layered legal shielding across brand recognition and unique aesthetic packaging.";
    importantConsiderations = [
      "Must establish novelty in design aesthetics",
      "Ensure non-functional shape requirements",
    ];
    documentsToPrepare = ["Design representation sheets", "Form TM-A"];
    possibleNextSteps = [
      "File Design registration with Kolkata Design Office",
      "Register brand name on IP India",
    ];
  }
    ...
    sources: [
      {
        ...
        excerpt:
          "Traditional knowledge and mere admixtures are barred from patent protection...",
      },
    ],
    disclaimer:
...
apiRouter.post("/abs/analyze", (req: Request, res: Response) => {
  ...
  res.json({
    ...
    abs_considerations: {
      ...
      statutory_sections: [
        "Section 3 (Foreign Access)",
        "Section 6 (IPR Approval)",
        "Section 7 (SBB Prior Intimation)",
        "Section 21 (Benefit Sharing)",
      ],
      benefit_sharing_rate:
        "0.1% to 0.5% on annual gross ex-factory sale of product (or 3% to 5% of purchase price of biological resources)",
      exemptions_applicable:
        "Registered AYUSH practitioners and cultivated medicinal plants for domestic commercial use are exempted under the 2023 Amendment Act.",
    },
    prior_art_tk_considerations:
      "CSIR-TKDL records may contain prior art citations. Disclosing geographical source and community origin is mandatory under Section 10(4) of Patents Act and 2024 WIPO GRATK Treaty.",
    ...
    recommended_next_steps: [
      isIPR
        ? "File Form III on NBA portal (Chennai) immediately after patent application number is generated."
        : "Submit prior intimation to the concerned State Biodiversity Board.",
    ...
    sources: [
      {
        ...
        source: "Gazette of India",
        excerpt:
          "Prior approval of NBA is mandatory before applying for IPR in or outside India...",
      },
    ],
  });
});
...
apiRouter.get("/research/search", (req: Request, res: Response) => {
  ...
  if (authority && authority !== "All") {
    results = results.filter((d) =>
      d.authority.toLowerCase().includes(authority.toLowerCase()),
    );
  }
  if (query) {
...
apiRouter.post("/admin/documents/:id/index", (req: Request, res: Response) => {
  res.json({
    success: true,
    id: req.params.id,
    message: "Document successfully re-indexed into semantic store.",
  });
});
